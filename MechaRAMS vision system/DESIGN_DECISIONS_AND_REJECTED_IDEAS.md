...
| Single-tag trig solve — gyro heading + tag range/bearing → XY (6328; upstream in PhotonVision as `PNP_DISTANCE_TRIG_SOLVE`; 1678 C2026 production) | ✅ (2026-07-16, toggleable) | `SingleTagTrigSolver` + `SingleTagStrategy.TRIG_SOLVE` behind the IO layer (keeps replay + explicit heading source, unlike adopting `PhotonPoseEstimator` wholesale). Default stays PNP until the A/B plan passes (test plan "2026-07-16"). σθ stays +∞ — never fuse our own heading back. |
| Constrained SolvePnP — flat-floor + heading-prior refinement (PhotonVision 2026, roboRIO-side ≤2 ms) | ❌ (documented) | Needs the `PhotonPoseEstimator`/`estimateConstrainedSolvepnpPose` API path, which bypasses our IO-layer replay shape. **Revisit** after the trig-solve A/B: if single-tag XY is still the limiter, this is the next step (docs-recommended flow: coproc multitag → fallback → constrained refine). |
| Anisotropic log-fitted covariance — power-law σ parallel/perpendicular to the camera→tag ray (5940 2026) | ✅ (2026-07-16, provisional + toggleable) | `VisionPolicy.anisotropicStandardDeviations` + `CovarianceModel.ANISOTROPIC`. Coefficients PROVISIONAL until fitted from robot logs (test plan stage R2); ISOTROPIC stays default. 5940's θ-blending deferred until we have fitted data. |
| QuestNav (Quest headset VIO odometry) | ❌ | **Excluded by mentor 2026-07-16**: localization quality was fine but the headset is too big for the robot. Camera-based (PhotonVision) only. **Revisit** only if packaging changes. |
...
| PathPlanner `SwerveSetpointGenerator` (254-derived; friction/torque-limited setpoints → no wheel slip → cleaner odometry) | ❌ (deferred 2026-07-16) | Real benefit appears only with measured robot parameters (mass/MOI/wheel COF/current limits) — on the bench with placeholder values it would just be another unvalidated model in the control path. **Revisit** right after drivetrain characterization (test plan R0/Characterization); integration point is the `ApplyRobotSpeeds` consumer in `configurePathPlanner` + the precision command. |
| Lookahead-interpolated pose input to the precision controller (1678 `PIDToPoseCommand`) | ❌ (noted) | Small latency-compensation refinement; our settle-gate design already matches 1678's. **Revisit** if robot logs show consistent lag-shaped overshoot in `DriveToPose/*`. |
